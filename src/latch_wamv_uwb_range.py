...
class LatchWAMVRange:
    def __init__(self):
        ...
        self.ctl_factor = {"x": 1.0, "y": 1.0, "z": 0.8, "yaw": 1.0}

        # self.goal = np.array([0.15, 0.1, 1.5, 0.0])
        self.goal = np.array([-0.5, 0.0, 0.72, 0.0])
    ...
    def cb_pose(self, msg):
        ...
        self.ctl_factor["x"] = min(1.0, abs(position.x) * 5)
        self.ctl_factor["y"] = min(1.0, abs(position.y) * 5)
        ...
        final_x = -0.2
        if (
            abs(position.x + 0.3 - self.goal[0]) < 0.2
            and abs(position.y - self.goal[1]) < 0.1
            and abs(position.z - 1.3 - self.goal[2]) < 0.1
        ):
            if self.goal[0] < final_x:
                self.goal[0] += 0.002
                self.goal_range = self.cal_drone_pose_to_uwb_range(self.goal)

            if abs(position.x + 0.3 - final_x) < 0.1:
                self.mavros_cmd_command()
        # if abs(position.x) < 0.5 and abs(position.y) < 0.5:
        #     self.drone_ver_rod_cmd.publish(Float64(data=-0.2))
        print(position.x - self.goal[0])
...
if __name__ == "__main__":
    ...
    start = time.time()
    rospy.init_node("latch_wamv_uwb_range")
    print(f"Time to init node: {time.time() - start}")
    np.set_printoptions(precision=3, suppress=True)
    uav_eval = LatchWAMVRange()
    rospy.spin()
